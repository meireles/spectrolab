...
You notice that `Acer_example` contains white reference measurement (the horizontal line around 1) and one measurement where something obviously went wrong (a dip at the NIR shoulder). During the measurement campaign those two measurement were flagged with "_WR" and "_BAD". With  `read_spectra()` it's easy to exclude flagged measurements. 
...
plot(acer_spectra)
```
Flagging "unusual" files during a measurement campaign is a very good idea, as it clearly helps you to quickly sort through your files when it comes to processing. However, you can also exclude "unusual" files lateron. We show you some options in section `Excluding "unusual" spectra`. 

### Create spectra from a matrix

If you already have your spectra in a matrix, you can use the function `as.spectra()` to convert it to a `spectra` object. The matrix **must** be in a very specific format, so **read carefully!**. Samples must be in rows and wavelengths in columns. The first column must be a sample label. In case you are as terrified as I am of loosing metadata for 100's of thousands of measurements consider including "all" crucial sample information in the filename (`species_plotID_plantID`). Column names must be the wavelength labels. We included an example of a matrix like that named `spec_matrix_example`.
...
Some instruments allow you to preserve the sensor overlap regions. This results in duplicated wavelenghts and a "jump" between the VIS/NIR and NIR/SWIR regions. The `jump corrector` removes the overlap regions between VIS/NIR and NIR/SWIR, respectively and adjusts the spectral regions to allow a smooth transition, while keeping as information as possible. In principle, the algorithm searches for the wavelengths of closest approach in both overlap regions, cuts the spectrum into three pieces, removes the overlap and finds a multiplier to paste the junks back together. No additional smoothing is applied. As a convention, the NIR regions is left unchanged.  
...
The `spectra` class holds the essential information used in spectral dataset: reflectance, wavelengths, etc. The class has a bunch of requirements in terms of both format and values. For instance, relative reflectance must be between 0 and 1 and wavelengths can not be duplicated.

If your data is in a matrix or data.frame with the same format as `Acer_juco` or `Acer_smoo` (check previous section for details), you can construct a `spectra` object by calling the `as.spectra()` function.
...
You can explore your `spectra` using `plot()`. 
...
For example, I often want to exclude the noisy regions at the beginning and end of the spectrum and subset the data to specific entries.   
