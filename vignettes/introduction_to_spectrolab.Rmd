...
You can read raw spectra files using the function `read_spectra()`. As of now, only files in the `.sig` format (SVC, Spectra Vista Corporation) can be read direcly, but other formats are in the works.
...
When plotting `(acer_spectra)` you will notice that `Acer_example` contains one white reference measurement (the horizontal line around 1.0 reflectance) and one obviously bad measurement (with the dip at the NIR shoulder). During the measurement campaign those two measurement were flagged with "_WR" and "_BAD" (run: `names(acer_spectra)`). With `read_spectra()` it's easy to exclude flagged files, no matter what flags you used.
...
plot(acer_spectra)
```
Flagging "unusual" files during a measurement campaign is a very good idea, as it clearly helps you to quickly sort through your files when it comes to processing. However, you can also exclude "unusual" files later on. We show you some build in options and how to filter out spectra based on your own criteria in section `Excluding "unusual" spectra` below. 

### Create spectra from a matrix

If you already have your spectra in a matrix, you can use the function `as.spectra()` to convert it to a `spectra` object. The matrix **must** be in a very specific format, so **read carefully!** (1) Samples must be in rows and wavelengths in columns. (2) The first column must be a sample label or filename. Note: In case you are as terrified as our virtual *Ms. A* of loosing metadata for 100's of thousands of spectra, consider including "all" crucial sample information in the filename during your measurement campaign (e.g. `species_plotID_plantID`). (3) Column names must be the wavelength labels. 

We included an example of a matrix like that named `spec_matrix_example`.
...
Treating each spectrum as a vector and standarizing it to unity length is a great way to remove brightness differences between measurements (e.g. due to changing lamp power), while preserving the shape of the spectrum (it's information content). Vector normalization is implemented in `spectrolab`'s `normalize_spectra()` function. Note: If you want to use *Ms. A's* trait coefficients, you need to use vector normalized spectra, limited to the 400 - 2400 nm wavelength range as input.
...
Depending on your analysis, it can be a good idea to smooth remaining noise at the sensor overlap regions. `spectrolab` comes with two implemented smoothing functions: `smoo_visnir()` uses moving average between 970 - 1025 nm. `smoo_nirswir()` uses linear interpolation between 1903 - 1917 nm. Note: *Ms. A* does not do this when predicting traits from spectra. 
...
## Excluding "unusual" spectra ... coming soon

The `spectrolab` package comes with a couple of functions to exclude bad spectra.
