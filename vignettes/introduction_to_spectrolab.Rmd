...
First you let us read in some raw data. 
...
You can take inspect the raw data with `plot_rawspec()`. 
...
You see that the sensor overlap regions were preserved when the data was acquired, resulting in duplicated wavelenghts and a "jump" between the VIS/NIR and NIR/SWIR regions. To construct a `spectra` object, you need to first run the jump corrector by calling `jump_corr()`.
...
We will use an example dataset called `spec_matrix_example`. Samples are in rows, and wavelengths in columns, and the first column is the sample name (in this case, a species name). The column names match wavelength labels. I tried to format `spec_matrix_example` as a typical spectral dataset in `csv` format.
