...
The goal of `spectrolab` is to provide 1) functions to read, process and visualize data from portable spectrometers as well as 2) a common interface other packages can build on. `spectrolab` introduces a `spectra` S3 class and implements various methods to read spectral files, access and subset spectal data, perform transformations such as vector normalization, smoothing, and sensor overlap matching, plot spectra, etc. 
...
It may be the case that file names of undesirable spectra where flagged in the field, e.g with "_WR" (white reference) and "_BAD" (bad measurements). You can avoid importing those files to beging with by passing those flags to the argument `exclude_if_matches` in `read_spectra()`.
...
Flagging "unusual" measurements during data collection is a very good idea. However, you can also exclude bad spectra and outliers by subsetting your spectra object. We show you how this works in the section **Subsetting spectra** further below.
...
`spectrolab` also lets you query the components of the `spectra`, done with the functions `names()` for sample names, `wavelengths()` for wavelength labels, `reflectance()` for the reflectance matrix, and `meta()` for the associated metadata (in case you have any).
