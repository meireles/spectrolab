...
* Smooth, resample, normalize, and differentiate spectra
...
smoothing/derivatives, spectral indices, and the tidy
...
> detectors meet. That information rides along as the `sensor_info(x)` attribute
> rather than as a `spectra` slot, and it is what lets `match_sensors()` find
> the splice points without you having to remember the boundaries; see the
...
`resample()`, `match_sensors()`, spectral indices, ...)
...
* Want to smooth or differentiate spectra? See [Smoothing and derivatives](preprocessing.html).
