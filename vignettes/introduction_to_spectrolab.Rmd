...
If your data is in a matrix or data.frame with the same format as `Acer_juco` (check previous section for details), you can construct a `spectra` object by calling the `as.spectra()` function.
...
Once the bad measurements are removed create an object of class `spectra` by calling the `as.spectra()` function, and plot your spectra using `plot()`. 
