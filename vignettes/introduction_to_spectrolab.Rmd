...
## Beyond `+`/`-`/`*`/`/`: other overloaded operators

A `spectra` object behaves like ordinary numeric data for several more base R
functions, all returning a `spectra` (or, where that wouldn't make sense, a
plain matrix/vector) rather than erroring or -- worse -- silently returning
something wrong:
...
`quantity()`/`wavelength_unit()` -- minimal provenance describing what a
`spectra` object measures -- are covered next.
