...
### Combining and aggregating keep different halves of your data {#combining-and-aggregating-keep-different-halves-of-your-data}

`spec1 + spec2` (or `-`, `*`, `/`, `^`) works element-wise once both operands
are `spectra` with identical dimensions and band labels, but the two sides
are **not symmetric** in what they keep:

* if the sample names differ between `spec1` and `spec2`, the result's names
  are cleared to `NA` (with a warning) rather than silently picking one side;
* metadata is **only** kept from the left-hand side -- `spec2`'s metadata is
  silently dropped.
...
meaningful. Only the four arithmetic operators return `spectra`.
