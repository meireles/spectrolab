...
Unmatched spectra collected with a 3-sensor instrument, such as the SVC HR1024, may look like this:
...
Use the function `match_sensors` to splice the sensor overlap regions as shown below.

You must pass the boundary between sensors using the `splice_at` argument. It is **critical** that you get those bands right (or very close) and every instrument (even from the same vendor) is different. 
